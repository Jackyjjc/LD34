...
public class CardBehaviour : MonoBehaviour {
    ...
    public Card card
    {
        ...
        set {
            cardBacking = value;
            gameObject.transform.FindChild("Panel/Text").GetComponent<Text>().text = value.name;
            gameObject.transform.FindChild("Texts/Text").GetComponent<Text>().text = value.text;
            if (value.textTable.ContainsKey("discardText")) {
                GameObject discardTextObj = gameObject.transform.FindChild("Texts/DiscardText").gameObject;
                discardTextObj.SetActive(true);
                discardTextObj.GetComponent<Text>().text = value.textTable["discardText"];
            }
            gameObject.transform.FindChild("CardTypeText").GetComponent<Text>().text = value.stage;
        }
    }
